...
Mesh Model::process_mesh(aiMesh* mesh, const aiScene* scene)
{
    ...
    for (unsigned int i = 0; i < mesh->mNumVertices; i++)
    {
        ...
        glm::vec2 uv_coords = mesh->mTextureCoords[0] ? glm::vec2(mesh->mTextureCoords[0][i].x, mesh->mTextureCoords[0][i].y) : uv_coords = glm::vec2(0.0f, 0.0f);

        Vertex vertex(position, normal, uv_coords, tangent, bitangent);
